...
enum TaskKind {
    Delete { node: NodeId, name: String, size: u64, mode: DeleteMode },
    Refresh { node: NodeId },
}
...
impl DiskTreeApp {
    pub fn new(cc: &eframe::CreationContext<'_>, renderer: &'static str, path: Option<PathBuf>) -> Self {
        setup_style(&cc.egui_ctx);
        let (tx, rx) = mpsc::channel();
        let ctx = cc.egui_ctx.clone();
        let spawned = std::thread::Builder::new().name("disktree-worker-drives".into()).spawn(move || {
            let drives = ops::fixed_drives();
            let _ = tx.send(drives);
            ctx.request_repaint();
        });
        let drives = if spawned.is_ok() { Drives::Loading(rx) } else { Drives::Ready(Vec::new()) };
    ...
    fn toast(&mut self, text: impl Into<String>, error: bool) {
        self.toast = Some(Toast { text: text.into(), until: Instant::now() + Duration::from_secs(6), error });
    }
    ...
    fn start_scan(&mut self, ctx: &egui::Context, root: PathBuf, restore: Option<PathBuf>) {
        ...
        let spawned = std::thread::Builder::new().name("disktree-worker-scan".into()).spawn(move || {
        ...
        match spawned {
            Ok(_) => {
                ...
                self.dialog = None;
                self.scan = Some(ScanJob { root, progress, rx, started: Instant::now(), restore });
            }
    ...
    fn poll_scan(&mut self, ctx: &egui::Context) {
        ...
        let result = match job.rx.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => Err(ScanError::Internal("the scan thread stopped".into())),
        };
    ...
    fn set_tree(&mut self, ctx: &egui::Context, tree: Tree, restore: Option<PathBuf>) {
        ...
        ctx.send_viewport_cmd(egui::ViewportCommand::Title(format!("disktree — {}", tree.root_path.display())));
        if let Some(old) = self.tree.replace(tree) {
            // Freeing millions of nodes takes a moment; don't stall the UI.
            let _ = std::thread::Builder::new().name("disktree-worker-free".into()).spawn(move || drop(old));
        }
        self.selected = None;
        self.generation += 1;
    ...
    fn spawn_task(&mut self, ctx: &egui::Context, kind: TaskKind, label: String, job: impl FnOnce() -> TaskResult + Send + 'static) {
        let (tx, rx) = mpsc::channel();
        let c = ctx.clone();
        let spawned = std::thread::Builder::new().name("disktree-worker-task".into()).spawn(move || {
            let r = job();
            let _ = tx.send(r);
            c.request_repaint();
        });
    ...
    fn start_delete(&mut self, ctx: &egui::Context, c: ConfirmDelete) {
        let size = if self.metric == Metric::Allocated { c.allocated } else { c.logical };
        ...
        diag::log(&format!("delete requested ({:?}): {}", c.mode, c.path.display()));
        let (path, mode, owner) = (c.path.clone(), c.mode, self.owner_hwnd);
        self.spawn_task(
            ctx,
            TaskKind::Delete { node: c.node, name: c.name, size, mode },
            label,
    ...
    fn start_refresh(&mut self, ctx: &egui::Context, node: NodeId) {
        ...
        self.spawn_task(ctx, TaskKind::Refresh { node }, label, move || {
            ...
            TaskResult::Refresh(
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| scan::scan_subtree(&path, &p, scan::default_threads())))
    ...
    fn poll_task(&mut self, ctx: &egui::Context) {
        ...
        let result = match task.rx.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => match task.kind {
                TaskKind::Delete { .. } => TaskResult::Delete(DeleteOutcome::Failed("The delete worker stopped unexpectedly.".into())),
                TaskKind::Refresh { .. } => TaskResult::Refresh(Err("The refresh worker stopped unexpectedly.".into())),
            },
        };
        let task = self.task.take().expect("task present");
        match (task.kind, result) {
            (TaskKind::Delete { node, name, size, mode }, TaskResult::Delete(outcome)) => {
                ...
                match outcome {
                    DeleteOutcome::Deleted | DeleteOutcome::AlreadyGone => {
                        ...
                        let msg = match (outcome, mode) {
                            (DeleteOutcome::AlreadyGone, _) => format!("{name} was already gone; removed it from the map."),
                            (_, DeleteMode::RecycleBin) => format!("Moved {name} to the Recycle Bin ({}).", format::bytes(size)),
                            (_, DeleteMode::Permanent) => format!("Permanently deleted {name} ({} freed).", format::bytes(size)),
                        };
                        self.toast(msg, false);
                    }
                    DeleteOutcome::Cancelled => {
                        self.toast("Cancelled. Anything already removed is reflected after a refresh.", false);
                    ...
                    DeleteOutcome::Failed(why) => {
                        self.dialog = Some(Dialog::Message { title: "Could not delete".into(), body: format!("{name}\n\n{why}") });
                    }
                }
            }
            (TaskKind::Refresh { node }, TaskResult::Refresh(res)) => match res {
                ...
                Err(e) => {
                    ...
                    let gone = self.tree.as_ref().map(|t| !t.path_of(node).exists()).unwrap_or(false);
                    if gone {
                        if let Some(tree) = &mut self.tree {
                            tree.remove(node);
                            if tree.is_ancestor_or_self(node, self.current) {
    ...
    fn refresh_list(&mut self) {
        let Some(tree) = &self.tree else { return };
        let key = (self.current, self.metric, self.sort, self.sort_desc, self.generation);
        ...
        match self.sort {
            SortKey::Size => v.sort_by(|&a, &b| tree.node(b).size(metric).cmp(&tree.node(a).size(metric)).then(a.cmp(&b))),
            SortKey::Files => v.sort_by(|&a, &b| tree.node(b).files.cmp(&tree.node(a).files).then(a.cmp(&b))),
            SortKey::Name => {
    ...
    fn handle_keys(&mut self, ctx: &egui::Context, actions: &mut Vec<Action>) {
        ...
        let (mut up, mut enter, mut home, mut f5, mut copy, mut reveal, mut prev, mut next, mut esc, mut help) =
        ...
        ctx.input_mut(|i| {
            ...
            i.events.retain(|e| match e {
                Event::Key { key: Key::Delete, pressed: true, modifiers, .. } if delete.is_none() => {
                    delete = Some(modifiers.shift);
                    false
                }
                _ => true,
            });
            copy = i.events.iter().any(|e| matches!(e, Event::Copy)) || i.consume_key(Modifiers::COMMAND, Key::C);
        ...
        if let Some(permanent) = delete {
            if let Some(sel) = self.selected {
                actions.push(Action::Delete(sel, if permanent { DeleteMode::Permanent } else { DeleteMode::RecycleBin }));
            }
        }
...
impl eframe::App for DiskTreeApp {
    fn ui(&mut self, ui: &mut Ui, frame: &mut eframe::Frame) {
        ...
        let dropped: Option<PathBuf> =
            ctx.input(|i| i.raw.dropped_files.iter().map(|f| f.path().to_path_buf()).find(|p| p.is_dir()));
...
impl DiskTreeApp {
        ...
        egui::CentralPanel::default().frame(egui::Frame::new().fill(BG).inner_margin(24.0)).show(ui, |ui| {
            ui.vertical_centered(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(RichText::new(format!("Scanning {}", job.root.display())).size(22.0).strong().color(Color32::WHITE));
                });
                ui.add_space(22.0);
                let p = &job.progress;
                ...
                egui::Grid::new("scan-stats").num_columns(2).spacing([40.0, 10.0]).show(ui, |ui| {
                    ...
                    ui.end_row();
                    stat(ui, "Speed", &format!("{} files/s", format::count((files as f64 / secs.max(0.001)) as u64)));
                    stat(ui, "Unreadable", &format::count(bad));
                    ui.end_row();
                });
                ui.add_space(18.0);
                let cur = p.current();
                ui.add(egui::Label::new(RichText::new(cur).color(TEXT_DIM).size(12.0)).truncate());
                ui.add_space(22.0);
                if ui.add(egui::Button::new(RichText::new("Cancel").size(15.0)).min_size(Vec2::new(120.0, 32.0))).clicked() {
                    cancel = true;
                }
            });
        });
    ...
    fn browser(&mut self, ui: &mut Ui, actions: &mut Vec<Action>) {
        ...
        egui::CentralPanel::default().frame(egui::Frame::new().fill(BG).inner_margin(6.0)).show(ui, |ui| {
    ...
    fn status_bar(&mut self, ui: &mut Ui, actions: &mut Vec<Action>) {
        ...
        egui::Panel::bottom("status").frame(egui::Frame::new().fill(PANEL).inner_margin(egui::Margin::symmetric(10, 6))).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let s = &tree.stats;
                let text = format!(
                    "{} on disk ({} logical) · {} files · {} folders · {} links not followed · scanned in {} · {}",
    ...
    fn side_panel(&mut self, ui: &mut Ui, actions: &mut Vec<Action>) {
        ...
        egui::Panel::right("side")
            ...
            .show(ui, |ui| {
                ...
                let title = if self.current == Tree::ROOT { tree.root_path.display().to_string() } else { cur.name.to_string() };
                ui.add(egui::Label::new(RichText::new(title).size(17.0).strong().color(Color32::WHITE)).truncate());
                ui.label(
                    RichText::new(format!(
                        "{} · {} files · {} items",
                ...
                // Selection card
                egui::Frame::new().fill(CARD).corner_radius(6.0).inner_margin(10.0).show(ui, |ui| {
                    ui.set_width(ui.available_width());
                    match self.selected {
                        None => {
                            ui.label(RichText::new("Click an item in the map or the list to select it.").color(TEXT_DIM));
                            ui.label(RichText::new("Double-click a folder to open it.").color(TEXT_DIM));
                        }
                        Some(sel) => {
                            let n = tree.node(sel);
                            let path = tree.path_of(sel);
                            ui.add(egui::Label::new(RichText::new(&*n.name).strong().size(15.0).color(Color32::WHITE)).truncate());
                            ui.add(egui::Label::new(RichText::new(path.display().to_string()).size(11.5).color(TEXT_DIM)).wrap());
                            let kind = match n.kind {
                                NodeKind::Dir => format!("Folder · {} files", format::count(n.files)),
                            ...
                            ui.add_space(4.0);
                            ui.horizontal_wrapped(|ui| {
                                if n.is_dir() && ui.button("Open").on_hover_text("Zoom into this folder (Enter)").clicked() {
                                    actions.push(Action::Open(sel));
                                }
                                if ui.button("Show in Explorer").on_hover_text("Ctrl+E").clicked() {
                            ...
                            ui.horizontal_wrapped(|ui| {
                                ...
                                if ui
                                    .add_enabled(can, egui::Button::new(RichText::new("Delete permanently…").color(Color32::from_rgb(255, 170, 170))))
                                    .on_hover_text("Shift+Delete. Bypasses the Recycle Bin; asks twice.")
                                    .clicked()
                                {
                ...
                // Column headers
                let arrow = |k: SortKey| if self.sort == k { if self.sort_desc { " ↓" } else { " ↑" } } else { "" };
                ui.horizontal(|ui| {
                    let w = ui.available_width();
                    if ui.add_sized([w - 190.0, 20.0], egui::Button::new(format!("Name{}", arrow(SortKey::Name))).frame(false)).clicked() {
                        actions.push(Action::Sort(SortKey::Name));
                    }
                    if ui.add_sized([90.0, 20.0], egui::Button::new(format!("Size{}", arrow(SortKey::Size))).frame(false)).clicked() {
                        actions.push(Action::Sort(SortKey::Size));
                    }
                    if ui.add_sized([80.0, 20.0], egui::Button::new(format!("Files{}", arrow(SortKey::Files))).frame(false)).clicked() {
                        actions.push(Action::Sort(SortKey::Files));
                    }
                });
                ui.separator();
                ...
                if self.list.is_empty() {
                    ui.add_space(20.0);
                    let msg = if cur.is_unreadable() { "This folder could not be read (access denied)." } else { "This folder is empty." };
                ...
                if let Some(idx) = self.list_scroll_to.take() {
                    ...
                    let prev = ui.ctx().data(|d| d.get_temp::<f32>(Id::new("list-offset"))).unwrap_or(0.0);
                    let top = idx as f32 * row;
                    let offset = if top < prev { top } else if top + row > prev + view_h { top + row - view_h } else { prev };
                ...
                ui.ctx().data_mut(|d| d.insert_temp(Id::new("list-offset"), output.state.offset.y));
            });
    }
    ...
    fn treemap(&mut self, ui: &mut Ui, actions: &mut Vec<Action>) {
        ...
        if self.tiles_key != Some(key) {
            ...
            if ms > 50.0 {
                diag::log(&format!("treemap layout: {} tiles in {ms:.1} ms", self.tiles.len()));
            }
        }
        let painter = ui.painter_at(rect);
        painter.rect_filled(rect, 0.0, BG);
        let origin = rect.min;
        let to_screen = |r: &treemap::Rect| Rect::from_min_size(Pos2::new(origin.x + r.x, origin.y + r.y), Vec2::new(r.w, r.h));
        ...
        if self.tiles.is_empty() {
            ...
            painter.text(rect.center(), Align2::CENTER_CENTER, msg, FontId::proportional(16.0), TEXT_DIM);
            return;
        }
            ...
            match tile.kind {
                TileKind::Node(id) => {
                    ...
                    if n.is_dir() {
                        ...
                        if tile.has_header {
                            let text = format!("{}  {}", n.name, format::bytes(n.size(self.metric)));
                        ...
                        } else if tree.children(id).next().is_none() || r.width() < 24.0 || r.height() < 24.0 {
                        ...
                        if r.width() > 3.0 && r.height() > 3.0 {
                            painter.rect_stroke(r, 0.0, Stroke::new(1.0, shade(fill, 0.55)), StrokeKind::Inside);
                        }
                        label_tile(&painter, r, &n.name, n.size(self.metric), fill);
                    }
                }
                TileKind::Rest { count, size, .. } => {
                    ...
                    if r.width() > 60.0 && r.height() > 16.0 {
                        label_tile(&painter, r, &format!("{} smaller items", format::count(count as u64)), size, fill);
                    }
                }
            }
        }
        ...
        if let Some(sel) = self.selected {
            if let Some(t) = self.tiles.iter().find(|t| t.kind == TileKind::Node(sel)) {
                painter.rect_stroke(to_screen(&t.rect), 0.0, Stroke::new(2.5, SELECT), StrokeKind::Inside);
            }
        }
        ...
        if let Some(t) = hovered {
            painter.rect_stroke(to_screen(&t.rect), 0.0, Stroke::new(1.5, Color32::WHITE), StrokeKind::Inside);
        }
        ...
        if response.double_clicked() {
            if let Some(TileKind::Node(id)) = hovered.map(|t| t.kind) {
                actions.push(Action::Open(id));
            }
            ...
            response.on_hover_ui_at_pointer(|ui| {
                ...
                match t.kind {
                    TileKind::Node(id) => {
                        ...
                        ui.label(RichText::new(tree.path_of(id).display().to_string()).size(11.5).color(TEXT_DIM));
                        ui.label(format!(
                            "{} on disk · {} logical",
                            format::bytes(n.allocated),
                            format::bytes(n.logical)
                        ));
                        let parent_name = if n.parent == Tree::ROOT { tree.root_path.display().to_string() } else { parent.name.to_string() };
                        ui.label(format!("{} of {parent_name}", format::percent(n.size(metric), parent.size(metric))));
                        if n.parent != current {
                            ui.label(format!(
                        ...
                        match n.kind {
                            NodeKind::Dir => {
                                ...
                                ui.label(RichText::new("Double-click to open · right-click for actions").size(11.0).color(TEXT_DIM));
                            }
                            NodeKind::File => {
                                ui.label(RichText::new(n.category.label()).size(11.5).color(TEXT_DIM));
                            }
                            NodeKind::Link => {}
                        }
                    }
                    TileKind::Rest { parent, count, size } => {
                        ui.label(RichText::new(format!("{} smaller items", format::count(count as u64))).strong());
                        ui.label(format!("{} in {}", format::bytes(size), tree.path_of(parent).display()));
                        ui.label(RichText::new("Too small to draw individually; open the folder to see them.").size(11.0).color(TEXT_DIM));
                    }
                }
            });
        }
    ...
    fn dialogs(&mut self, ctx: &egui::Context) {
        ...
        let unreadable: Vec<(String, String)> =
            self.tree.as_ref().map(|t| t.stats.error_samples.clone()).unwrap_or_default();
            ...
            match dialog {
                ...
                Dialog::Confirm(c) => {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        let fill = if permanent && c.step > 0 { DANGER } else { Color32::from_rgb(60, 90, 140) };
                        let button = egui::Button::new(RichText::new(label).color(Color32::WHITE)).fill(fill).min_size(Vec2::new(150.0, 30.0));
                        if ui.add_enabled(typed_ok, button).clicked() {
                        ...
                        if ui.add(egui::Button::new("Cancel").min_size(Vec2::new(90.0, 30.0))).clicked() {
                            close = true;
                        }
                    });
                }
    ...
    fn paint_toast(&mut self, ctx: &egui::Context) {
        ...
        let painter = ctx.layer_painter(egui::LayerId::new(egui::Order::Foreground, Id::new("toast")));
        let galley = painter.layout(t.text.clone(), FontId::proportional(14.0), Color32::WHITE, 560.0);
        let size = galley.size() + Vec2::new(28.0, 18.0);
        let rect = Rect::from_center_size(Pos2::new(screen.center().x, screen.max.y - 90.0), size);
        painter.rect_filled(rect, 8.0, if t.error { Color32::from_rgb(120, 40, 40) } else { Color32::from_rgb(40, 60, 90) });
...
const SHORTCUTS: &[(&str, &str)] = &[
    ...
    ("Backspace / Alt+Up", "Up one level (also mouse Back button, middle-click)"),
...
fn warning_box(ui: &mut Ui, text: &str) {
    egui::Frame::new().fill(Color32::from_rgb(80, 28, 28)).corner_radius(6.0).inner_margin(8.0).show(ui, |ui| {
        ui.set_width(ui.available_width());
...
fn drive_card(ui: &mut Ui, d: &Drive) -> bool {
    ...
    let title = if d.label.is_empty() { root.clone() } else { format!("{root}  {}", d.label) };
    p.with_clip_rect(rect.shrink(8.0)).text(rect.min + Vec2::new(14.0, 12.0), Align2::LEFT_TOP, title, FontId::proportional(17.0), Color32::WHITE);
    if d.total > 0 {
        let used = d.total.saturating_sub(d.free);
        let frac = used as f32 / d.total as f32;
        let bar = Rect::from_min_size(rect.min + Vec2::new(14.0, 44.0), Vec2::new(rect.width() - 28.0, 8.0));
        p.rect_filled(bar, 4.0, Color32::from_rgb(55, 60, 74));
        let fill = Rect::from_min_size(bar.min, Vec2::new(bar.width() * frac, bar.height()));
        p.rect_filled(fill, 4.0, if frac > 0.9 { Color32::from_rgb(229, 83, 83) } else { ACCENT });
        let text = format!("{} free of {}{}", format::bytes(d.free), format::bytes(d.total), if d.filesystem.is_empty() { String::new() } else { format!(" · {}", d.filesystem) });
        p.text(rect.min + Vec2::new(14.0, 62.0), Align2::LEFT_TOP, text, FontId::proportional(12.5), TEXT_DIM);
    } else {
        p.text(rect.min + Vec2::new(14.0, 50.0), Align2::LEFT_TOP, "Click to scan", FontId::proportional(12.5), TEXT_DIM);
    }
...
fn breadcrumbs(ui: &mut Ui, tree: &Tree, current: NodeId, actions: &mut Vec<Action>) {
    ...
    for (i, &n) in chain.iter().enumerate().skip(skip) {
        ...
        let name = if n == Tree::ROOT { tree.root_path.display().to_string() } else { tree.node(n).name.to_string() };
        let is_current = n == current;
        let text = if is_current { RichText::new(name).strong().color(Color32::WHITE) } else { RichText::new(name).color(ACCENT) };
...
fn list_row(ui: &mut Ui, n: &disktree::tree::Node, metric: Metric, parent_size: u64, selected: bool) -> egui::Response {
    ...
    let frac = if parent_size > 0 { size as f32 / parent_size as f32 } else { 0.0 };
    // Share-of-folder bar behind the name.
    let bar_w = (rect.width() - 190.0).max(0.0) * frac.clamp(0.0, 1.0);
    if bar_w > 0.5 {
        p.rect_filled(Rect::from_min_size(rect.min + Vec2::new(0.0, ROW_H - 3.0), Vec2::new(bar_w, 2.0)), 0.0, Color32::from_rgb(70, 110, 170));
    }
    let icon = Rect::from_min_size(rect.min + Vec2::new(4.0, 6.0), Vec2::new(11.0, 10.0));
    match n.kind {
        NodeKind::Dir => {
            p.rect_filled(Rect::from_min_size(icon.min, Vec2::new(5.0, 3.0)), 1.0, Color32::from_rgb(220, 180, 90));
            p.rect_filled(Rect::from_min_max(icon.min + Vec2::new(0.0, 2.0), icon.max), 1.5, Color32::from_rgb(220, 180, 90));
        }
        NodeKind::File => {
        ...
        NodeKind::Link => {
            p.rect_stroke(icon.shrink(1.0), 2.0, Stroke::new(1.0, TEXT_DIM), StrokeKind::Inside);
        }
    }
    let name_rect = Rect::from_min_max(rect.min + Vec2::new(22.0, 0.0), Pos2::new(rect.max.x - 178.0, rect.max.y));
    ...
    let color = if n.kind == NodeKind::Link || n.is_unreadable() { TEXT_DIM } else { Color32::from_rgb(225, 228, 235) };
    p.with_clip_rect(name_rect).text(Pos2::new(name_rect.min.x, rect.center().y), Align2::LEFT_CENTER, name, FontId::proportional(13.5), color);
    p.text(Pos2::new(rect.max.x - 92.0, rect.center().y), Align2::RIGHT_CENTER, format::bytes(size), FontId::proportional(13.0), Color32::WHITE);
    p.text(Pos2::new(rect.max.x - 50.0, rect.center().y), Align2::RIGHT_CENTER, format::percent(size, parent_size), FontId::proportional(12.0), TEXT_DIM);
    let files = if n.is_dir() { format::count(n.files) } else { String::new() };
    p.text(Pos2::new(rect.max.x - 2.0, rect.center().y), Align2::RIGHT_CENTER, files, FontId::proportional(12.0), TEXT_DIM);
    resp
}
...
fn context_menu(ui: &mut Ui, id: NodeId, kind: NodeKind, busy: bool, actions: &mut Vec<Action>) {
    ...
    if kind == NodeKind::Dir && ui.add_enabled(!busy, egui::Button::new("Refresh this folder")).clicked() {
        actions.push(Action::RefreshFolder(id));
        ui.close();
    }
    ui.separator();
    let can = !busy && kind != NodeKind::Link;
    if ui.add_enabled(can, egui::Button::new("Move to Recycle Bin…")).clicked() {
        actions.push(Action::Delete(id, DeleteMode::RecycleBin));
        ui.close();
    }
    if ui.add_enabled(can, egui::Button::new("Delete permanently…")).clicked() {
...
fn label_tile(painter: &egui::Painter, r: Rect, name: &str, size: u64, fill: Color32) {
    ...
    let text_color = if luminance(fill) > 0.55 { Color32::from_rgb(20, 20, 24) } else { Color32::WHITE };
    let clip = painter.with_clip_rect(r.shrink(2.0));
    if r.height() >= 32.0 {
        clip.text(Pos2::new(r.min.x + 5.0, r.min.y + 4.0), Align2::LEFT_TOP, name, FontId::proportional(12.0), text_color);
        clip.text(Pos2::new(r.min.x + 5.0, r.min.y + 18.0), Align2::LEFT_TOP, format::bytes(size), FontId::proportional(11.0), text_color.gamma_multiply(0.8));
    } else {
        clip.text(Pos2::new(r.min.x + 4.0, r.center().y), Align2::LEFT_CENTER, name, FontId::proportional(11.0), text_color);
    }
}
